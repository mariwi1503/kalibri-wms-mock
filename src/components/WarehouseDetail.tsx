...
export const WarehouseDetailModal = ({ warehouse, onClose, formatPrice }: Props) => {
    if (!warehouse) return null;

    // Convert phone number to clean format for WhatsApp link
    const waPhone = warehouse.owner_phone ? warehouse.owner_phone.replace(/\D/g, '') : '';
                ...
                <div className="relative">
                    ...
                    <div className="h-60 bg-linear-to-br from-gray-200 to-gray-400 relative rounded-t-2xl">
                        ...
                        <div className="absolute inset-0 bg-black/40 flex flex-col justify-end p-6">
                            <h2 className="text-3xl font-bold text-white mb-2 leading-tight">{warehouse.name}</h2>
                            <div className="flex items-center text-blue-100">
                                <MapPin className="w-5 h-5 mr-2" />
                                <span className="text-lg">{warehouse.address}, {warehouse.city}, {warehouse.province}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-6 sm:p-8 space-y-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
                        <div className="lg:col-span-1">
                            <h3 className="text-2xl font-bold text-gray-900 mb-4 border-b pb-2">Deskripsi Gudang</h3>
                            <p className="text-gray-700 text-base leading-relaxed">{warehouse.description}</p>
                        </div>
                        <div className="lg:col-span-1 bg-blue-50 p-5 rounded-xl border border-blue-200">
                            <h3 className="text-xl font-bold text-blue-800 mb-4">Informasi Utama</h3>
                            <div className="space-y-3 text-gray-700">
                                <div className="flex justify-between items-center border-b border-blue-100 pb-2">
                                    <span className="font-medium">Luas:</span>
                                    <span className="font-semibold text-blue-600">{warehouse.size_sqm} m²</span>
                                </div>
                                <div className="flex justify-between items-center border-b border-blue-100 pb-2">
                                    <span className="font-medium">Lokasi:</span>
                                    <span className="font-semibold">{warehouse.city}, {warehouse.province}</span>
                                </div>
                    ...
                    {/* Fasilitas */}
                    <div>
                        <h3 className="text-2xl font-bold text-gray-900 mb-4 border-b pb-2">Fasilitas</h3>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {warehouse.facilities.map((facility, index) => (
                                <div key={index} className="flex items-center space-x-2 text-gray-700 p-2 bg-green-50 rounded-lg">
                    ...
                    <div className="pt-4 border-t border-gray-100">
                        <h3 className="text-2xl font-bold text-gray-900 mb-4">Harga & Kontak</h3>
                        <div className="flex flex-col md:flex-row justify-between items-center p-6 bg-linear-to-r from-blue-600 to-green-600 rounded-xl text-white shadow-xl">
                            <div className="mb-4 md:mb-0">
                                <p className="text-xl font-semibold opacity-90">Harga Sewa Bulanan</p>
                                <span className="text-4xl font-extrabold">{formatPrice(warehouse.price_per_month)}</span>
